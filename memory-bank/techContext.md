...
| Web host | Blazor Web App, Interactive Server render mode |
...
- `src/Game.Engine/Game.Engine.csproj` — `Microsoft.NET.Sdk`, net10.0, nullable+implicit
  usings. Currently only placeholder `Class1.cs`.
...
- `src/Game.Web/Game.Web.csproj` — `Microsoft.NET.Sdk.Web`, net10.0; refs Game.UI;
  `BlazorDisableThrowNavigationException=true`. `Program.cs` uses `MapStaticAssets()`,
  `AddInteractiveServerComponents/RenderMode`, status-code re-execute to `/not-found`.
